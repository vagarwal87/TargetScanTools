...
use Inline Python => q{from BranchLengthScoring import BBLS,BLS,getMaxBLS,parse_tree}; #import and export python tree fxns
use Env; Env::import();

@ISA = qw(Exporter);
@EXPORT = qw(ceil floor min max qnorm uniq
		 bsub qsub parallelize 
		 fisher_yates_shuffle log2 log10 mean median medianabsdev quantile round variance stdev sum trimmed_mean trimmed_stdev vecsum zscore 
		 com comRNA fold gen_all_nmers readFasta rev revCom revComRNA gcContent 
		 BBLS BLS getMaxBLS parse_tree
	...
	foreach $file (@files){
		...
		if (!$qsub && !$bsub) {
			...
			if ($cmd =~ /bin_MSA/ && $cmd !~ /all/){
				...
				($bin) = (split /\s/, `grep -m1 -P '^$code\t' $species/$region/allgenes.bins`)[-1] if $kmerlen == 2 || $kmerlen == 8;
